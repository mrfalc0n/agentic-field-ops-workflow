...
This is not a chatbot demo. It is a practical operating system for a messy business workflow.
...
The planned agent layer follows a controlled loop:

```text
Observe -> classify -> normalize -> detect exceptions -> propose -> approve -> execute -> audit
```
...
- Accounting control agent.

Each agent has bounded tools and clear approval gates. The goal is not autonomy theater. The goal is useful automation with guardrails.
